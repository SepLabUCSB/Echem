...
# data_dir = r'C:\Users\BRoehrich\Desktop\EIS-EAB data\2022-11-16\first 30 min -340mV'
data_dir = r'C:\Users\BRoehrich\Desktop\EIS-EAB data\2022-11-28\fast recording additions'
...
def make_meisp_folders(data_dir):
    ...
    for file in os.listdir(data_dir) :
        if (file.endswith('s.txt') and ~file.endswith('currents.txt')):
            i += 1
    ...
    for i in range(no_of_folders):
        ...
        for j in range(idx, idy):
            try:
                src = os.path.join(data_dir, f'{j:06}s.txt')
                dst = os.path.join(new_folder, f'{j:06}s.txt')
...
# folder_numbers = folder_numbers[:-3]
...
# file = output
# file = r'C:/Users/BRoehrich/Desktop/EIS-EAB data/2022-11-16/dosing -340mV/meisp_fits.xlsx'
...
fig, ax = plt.subplots()
y = signal.savgol_filter(np.array(df['ket']), 7, 1, mode='nearest')
ax.plot(df['time/s']/60, df['ket'], '.', color = colors[0], alpha=0.1)
ax.plot(df['time/s']/60, y, color = colors[0])
